...
def home(request):
	...
	return render(request,'base/index.html',{'data':data,'banners':banners})

# Category
def category_list(request):
    data=Category.objects.all().order_by('-id')
    return render(request,'base/category_list.html',{'data':data})

# Brand
def brand_list(request):
    data=Brand.objects.all().order_by('-id')
    return render(request,'base/brand_list.html',{'data':data})
...
def category_product_list(request,cat_id):
	...
	return render(request,'base/category_product_list.html',{
			'data':data,
			})
...
def product_detail(request,slug,id):
	...
	return render(request, 'base/product_detail.html',{'data':product,'related':related_products,'colors':colors,'sizes':sizes,'reviewForm':reviewForm,'canAdd':canAdd,'reviews':reviews,'avg_reviews':avg_reviews})
...
def checkout(request):
	...
	if 'cartdata' in request.session:
		...
		return render(request, 'base/checkout.html',{'cart_data':request.session['cartdata'],'totalitems':len(request.session['cartdata']),'total_amt':total_amt,'form':form,'address':address})

@csrf_exempt
def payment_done(request):
	returnData=request.POST
	return render(request, 'base/payment-success.html',{'data':returnData})
...
#     return render(request,'base/product/category.html', context)
...
#     return render(request,'base/store1.html', context)
...
# #     return render(request, 'base/products/category.html', {'category': category, 'products': products})


# # def product_detail(request, slug):
# #     product = get_object_or_404(Product, slug=slug, in_stock=True)
# #     return render(request, 'base/products/detail.html', {'product': product})
